 ...
 *
 *   海外CF価格 22万円
 *     → 卸値（海外価格の30〜50%）   6.6万〜11万円
 *     → 販売価格に対し卸値が3割になるよう逆算（30-30-30）
 *     → 日本CF価格                 22万〜37万円
 *
 * ■ 精度について
 ...
 * 卸値から日本CF販売価格までの倍率。
 *
 * 業界の目安として言われる「30-30-30」に基づく:
 *   仕入れ（卸値）  約30%
 *   送料・関税・認証 約30%
 *   自社利益        約30%
 * つまり販売価格に対して卸値が3割 → 卸値 ÷ 0.30 ≒ 3.33倍。
 *
 * ※ CF手数料（Makuake等で約20%）をこの内訳のどこで見るかは
 *    運用次第。実際の交渉で卸値が確定したら実測値に置き換えること。
 */
export const WHOLESALE_SHARE_OF_PRICE = 0.3;
export const LANDED_MULTIPLE = 1 / WHOLESALE_SHARE_OF_PRICE; // ≒ 3.33
